...
if [ "$FALLBACK_ENABLED" = "true" ]; then
  ...
  if [ "$FALLBACK_SOURCE" = "siliconflow_free" ]; then
    ...
    if [[ "$FALLBACK_SOURCE" == "openaicompatible" || "$FALLBACK_SOURCE" == "openai_compatible" || "$FALLBACK_SOURCE" == "gemini" ]]; then
      ...
      for SECRET_NAME in PUBLIC_FALLBACK_LLM_API_KEY PUBLIC_FALLBACK_LLM_API_KEY_2; do
        if ! printf '%s' "$SECRET_LIST" | grep -q "\"name\"[[:space:]]*:[[:space:]]*\"${SECRET_NAME}\""; then
          echo "❌ $FALLBACK_SOURCE requires $SECRET_NAME for the two-account public free-tier pool, but the Worker secret is missing."
          exit 1
        fi
      done
      echo "✅ Both public fallback API keys are configured; they will be tried in order before native siliconflow_free."
    fi
  fi
