...
app.post('/agent/jobs/:id/progress', async (c) => {
  ...
  if (status === 'failed') {
    await c.env.DB.prepare(
      `UPDATE jobs SET status = 'failed', finished_at = ?, progress_percent = ?, progress_phase = ?, error_message = ?, log_tail = ?, execution_metadata = coalesce(?, execution_metadata), active_provider_name = coalesce(?, active_provider_name) WHERE id = ? AND status = 'running'`
    ).bind(now, newPercent, phase, errorMsg, logTail, executionMetadata, activeProviderName, id).run()
